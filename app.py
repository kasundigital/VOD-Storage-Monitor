...
, diff, re.M) if diff else None
        pending[key]=int(m.group(1)) if m else 0

    warning_source = status + ('\n'+diff if diff else '')
    warning=list(dict.fromkeys(x.strip() for x in warning_source.splitlines() if 'WARNING!' in x))
    pending_total=sum(v for k,v in pending.items() if k!='equal')

    if maintenance_running:
        if job.get('name') == 'sync':
            protection='Syncing'
        else:
    ...
    return {'status_rc':rc,'diff_rc':rc2,'status_ok':status_ok,'diff_ok':diff_ok,
            ...
            'maintenance_running':maintenance_running,'job':job}
